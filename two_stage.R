...
vskt.mp <- select(vskt.mp, one_of(c("case", "gbja", "weight", "rente_2017_gesamt", "unempben", "expunempl", "divorced", "expwork", "ltearnings", "age","ja", "sex")))
...
vskt.tex <- select(vskt.mp, one_of(c("rente_2017_gesamt", "expunempl", "unempben", "expwork", "age", "divorced", "ltearnings")))
vskt.tex <- vskt.tex %>% 
  mutate(everdivorced = as.numeric(as.numeric(divorced)==2,0,1)) %>% 
  mutate(ltearnings = round(ltearnings)) %>% 
  mutate(rente_2017_gesamt = round(rente_2017_gesamt)) %>% 
...
forestSOEP <- randomForest(factor(education, ordered = T) ~ gbja + rente_2017_gesamt + expunempl + expwork + unempben + divorced + sex, data = soep.mp, importance = T, corr.bias = T)
...
forestVSKT <- randomForest(ltearnings ~ gbja + rente_2017_gesamt + expunempl + expwork + unempben + divorced + sex, data = vskt.mp , importance = T, corr.bias = T)
...
X.mtc <- c("rente_2017_gesamt","gbja" , "unempben", "expwork", "expunempl")
...
(X.mtc <- c("rente_2017_gesamt", "gbja", "expwork", "expunempl", "unempben")) #final X_M
